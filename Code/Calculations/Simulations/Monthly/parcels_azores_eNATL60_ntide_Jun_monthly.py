...
ufiles = sorted(glob(data_path + 'UV/' + tide_name + '/eNATL60ACOl-' + tide_nameF + '_y2010m06d*.1h_sozocrtx.nc'))
vfiles = sorted(glob(data_path + 'UV/' + tide_name + '/eNATL60ACOl-' + tide_nameF + '_y2010m06d*.1h_somecrty.nc'))
...
outdir = "/data/oceanparcels/input_data/eNATL60/Azores/outputs_parcels/"
...
for start_day in start_days:
       ...
       outname = outdir + "Particle_AZO_grid100000p_" + tide_name + "_" + str(nmonth).zfill(2) + str(start_day).zfill(2) + "_hourly_MONTH"
       pset_AZO_grid = ParticleSet(fieldset=fieldset_AZO, pclass=JITParticle, lon=lons, lat=lats, time=datetime(2010, nmonth, start_day, 0, 30)) 
